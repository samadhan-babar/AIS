...
typedef struct
{
    uint32_t        magic = 0xABADBABE;           // Magic value to indicate valid data (default: 0)
    uint32_t        mmsi = 123456789;            // Vessel MMSI (default: 0)
    char            name[21] = "TEST TRANSMISSION";      // Vessel name (all caps) (default: empty string)
    char            callsign[8] = "BHARAT";   // Radio station call sign (default: empty string)
    uint8_t         len = 0;             // Length in meters (default: 0)
    uint8_t         beam = 0;            // Beam in meters (default: 0)
    uint8_t         bowOffset = 0;       // Distance between bow and GPS antenna (default: 0)
    uint8_t         portOffset = 0;      // Distance between port side and GPS antenna (default: 0)
    VesselType      type = VESSEL_TYPE_UNSPECIFIED;  // Vessel type (default: Undefined)
} StationData;
